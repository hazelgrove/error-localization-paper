...
it is unbound, so a second error can be simultaneously localized despite the missing information about \li{f}. Elm's compiler stops here.
...
If localization had assumed that the \li{then} branch correct, as for example in Elm and \li{merlin}, then \li{x : Bool} and an error should be reported on the left operand (though this error might mislead the programmer if this earlier localization guess was incorrect).
