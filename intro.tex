...
by observing recent editing history \cite{steady-typing}\todo{fix cite}, 
...
If localization had assumed that the \li{then} branch correct, as for example in \li{merlin}, then 
...
so we choose not to report an error in Hazel (and in fact treat \li{x}'s type as the unknown a.k.a. dynamic type from gradual typing).
