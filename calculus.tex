...
bidirectional type error localization. Namely, the calculus employs bidirectional and gradual typing
to provide a judgmental framework with a unified theory for the design of typing semantics that
support ill-typed programs. \Cref{sec:calculus-calculus} describes its form and metatheory by
example on a small gradually typed lambda calculus (GTLC) \cite{Siek06a}, extending it with binary
product types in \cref{sec:calculus-products}. All rules and theorems may be found in the
supplementary appendix, alongside a complete mechanization in the Agda proof assistant
\cite{norell:thesis} (discussed in \cref{sec:calculus-agda}). With products, we then consider how
error localization is complicated by patterned let expressions in \cref{sec:calculus-let}. Finally,
\cref{sec:calculus-poly} explores how the system might be extended to more complex typing features,
such as System F-style polymorphism.
...
and there is a single arithmetic operation of addition on numeric expressions. $\ETrue$ and
$\EFalse$ correspond to the boolean values $\textsf{true}$ and $\textsf{false}$, respectively, and
$\EIf{\EMV_1}{\EMV_2}{\EMV_3}$ gives a conditional structure.

In addition to these forms, $\TUnknown$ is the unknown type from gradual type theory \cite{Siek06a}.
Modelling the edit state of a program in development, $\EEHole$ denotes an \emph{expression hole},
used to represent syntactically incomplete portions of the program à la Hazelnut
\cite{HazelnutPOPL}. Note, however, that these are not critical to the calculus.
...
Types classify expressions by a \emph{bidirectional type system} \cite{BidirTyping}, which employs
two mutually defined judgments. \emph{Type synthesis}, written $\ctxSynTypeU{\ctx}{\EMV}{\TMV}$,
establishes that, under the typing context $\ctx$, the expression $e$ synthesizes or locally infers
\cite{Localinf} the type $\TMV$. \emph{Type analysis}, written $\ctxAnaTypeU{\ctx}{\EMV}{\TMV}$,
states that the expression $\EMV$ may appear where an expression of type $\TMV$ is expected. We may
give these typing semantics traditionally, without consideration for error localization and
recovery.
...
other semantic services. % TODO such as THI.
...
denote a different error message that might be shown by an editor or emitted by a compiler.

Throughout the remainder of this section, as we formulate marking for the GTLC, we will motivate and
give precise semantics for these error marks. Note that these constitute a non-exhaustive list;
depending on the syntactic forms and typing features found in a given language, other kinds of marks
may be necessary. Indeed, in \cref{sec:calculus-poly}, we consider the kinds of marks that may be
...
preserve syntactic structure modulo those marks. \cref{fig:calculus-mark-erasure} thus gives a
notion of \emph{mark erasure}, which converts marked expressions back into unmarked expression by
...
should match those synthesized or analyzed against in marking.

In the remainder of this subsection, we consider the different type errors that may arise in the
unmarked language while keeping in mind these two theorems; in this way, the traditional typing
semantics of the unmarked language guide the construction of the marking process.
...
the marking of holes. Like number literals and booleans, these are governed by straightforward
rules.
%
\begin{mathpar}
  \judgment{ }{
...
function. Indeed, given the algorithmic nature of bidirectional typing, it is fairly direct to extract
an implementation from these rules.
...
by adding binary product types, written $\TProd{\TMV}{\TMV}$. In the interest of space, we focus on
aspects related to marking; a comprehensive set of rules is found in the supplemental material.
...
arrow types, the judgment $\matchedProd{\TMV}{\TMV_1}{\TMV_2}$ establishes that $\TMV$ may be
considered the binary product type $\TProd{\TMV_1}{\TMV_2}$ [cite?].\todo{cite}
...
to the synthesis rules, an explicit analysis rules is defined for the former: if an expected type is
known, the responsibility for type errors is shift to the components.

The marked language is extended with the same forms, and we again devise marking rules by modelling
off the unmarked typing rules.
%
\begin{mathpar}
  \judgment{
...
polymorphism. Specifically, we consider System F-style explicit polymorphism.\todo{write me}
