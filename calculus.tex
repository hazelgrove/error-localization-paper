...
type error localization in bidirectional systems, a judgmental framework provides a unified theory
for the design of typing semantics that supports ill-typed programs. \Cref{sec:calculus-calculus}
describes its form and metatheory by example on a minimal gradually typed lambda calculus, extending
with binary product types in \cref{sec:calculus-products}. All rules and theorems may be found in
the supplementary appendix, alongside a complete mechanization in the Agda proof assistant, which is
discussed in \cref{sec:calculus-agda}. Finally, in \cref{sec:calculus-poly}, we consider how the
system might be extended to more complex typing features, such as System F-style polymorphism.
...
lambda calculus extended with numbers and booleans. Consider the syntax for such a language, as
shown in \cref{fig:calculus-syntax-uexp}. The base types $\TNum$ and $\TBool$ classify number and
boolean expressions. The number literal corresponding to the mathematical number $n$ is given by
$\ENumMV$, and there is a single addition operation on numeric expression. $\ETrue$ and $\EFalse$
...
is the unknown type from gradual type theory.
...
The language is typed bidirectionally, which employs two mutually defined judgments. \emph{Type
synthesis}, written $\ctxSynType{\ctx}{\EMV}{\TMV}$, establishes that, under the typing context
$\ctx$, the expression $e$ synthesises or infers the type $\TMV$. \emph{Type analysis}, written
$\ctxAnaType{\ctx}{\EMV}{\TMV}$, states that the expression $\EMV$ may appear where an expression of
type $\TMV$ is expected. We may give these typing semantics traditionally, without considering error
...
\begin{itemize}
  ...
  \item \textbf{Marking}, which \emph{marks} any unmarked expression into a marked expression,
    inserting error markings where appropriate.
\end{itemize}
%
...
process with error localization and recovery.

% TODO more intuition

We give, for now, only part of the marked expression syntax in \cref{fig:calculus-syntax-mexp-part}.
Throughout the remainder of this section, as we consider the formulation of marking, we will add the
necessary error mark forms.
...
\begin{figure}[htbp]
  ...
  \end{array}\]
  \caption{Partial syntax of marked expressions}
  \label{fig:calculus-syntax-mexp-part}
\end{figure}
...
\end{theorem}
%
That is, we may mark, i.e. type check and localize errors for \emph{any} syntactically well-formed
program. Furthermore, marking should preserve the syntactic structure of programs, modulo error
marks. Thus, we must define a notion of \emph{mark erasure}, which converts marked expressions back
into unmarked expression by removing error marks. \Cref{fig:calculus-mark-erasure-part} gives the
definition of mark erasure, written $\erase{\ECMV}$, for our initial marked expression syntax. Then,
we may define a \emph{well-formedness} criterion for marking:
%
...
marked expressions that are well-typed; indeed, their types should match the types synthesised or
analysed against in marking. % TODO more intuition for this theorem

\input{figs/calculus-mark-erasure-part}
...
% TODO subsumption is discussed later...
The typing of unmarked variables in the gradually typed lambda calculus is standard. Because of
subsumption, we need only define a synthesis rule:
\[%
  \judgment{
    \inCtx{\ctx}{x}{\TMV}
  }{
...
marking would convert the unmarked $x$ into the same marked $x$ via the following marking and typing
rules:
%
\begin{mathpar}
  \judgment{
...
bound? Then, \textsc{USVar} does not apply, and the typing semantics say nothing about such
expressions. To satisfy totality, such a possibility motivates an \emph{unbound variable mark}:
%
\[\begin{array}{rrcl}
  \ECMName & \ECMV & \Coloneqq & \cdots \mid \ECUnbound{x}
\end{array}\]
%
accompanied by the following rules:
%
\begin{mathpar}
  \judgment{
    \notInCtx{\ctx}{x}
...
checking to continue---instead of simply failing.

To prove \cref{thm:calculus-marking-well-formedness}, we extend the definition of mark erasure to
handle the new mark:
%
\[\begin{array}{rcl}
  & \cdots & \\
...
There are explicit synthesis and analysis rules for lambda abstractions:
%
\begin{mathpar}
  \judgment{
...
to the syntax we add an \emph{inconsistent type mark}, which indicates that the expression does not
synthesize a type consistent with the expected type:
%
\[\begin{array}{rrcl}
  \ECMName & \ECMV & \Coloneqq & \cdots \mid \ECInconType{\ECMV}
\end{array}\]
%
along with the following rules, where the lambda abstraction is placed inside the inconsistent type
mark.
%
\begin{mathpar}
  \judgment{
...
Finally, as with the unbound variable mark, we extend mark erasure:
%
\[\begin{array}{rcl}
  & \cdots & \\
...
Again, to satisfy totality, we must consider the case when $\TMV$ is not a matched arrow type. There
is no expected type for the argument, so we perform analytic marking on $\EMV_2$ against the
unknown type. The applied expression is placed inside the inconsistent type mark, which is in
accordance with the meaning of that mark.
%
\begin{mathpar}
  \judgment{
    \ctxSynFixedInto{\ctx}{\EMV_1}{\ECMV_1}{\TMV} \\
...
unmatched types, such as binary products. Indeed, in \cref{sec:calculus-products}, we opt for this
approach. Another approach is to mark an error around the entire application expression.

\subsubsection{Numbers}
\label{sec:calculus-numbers}

The marking and typing of numbers is straightforward:
%
\begin{mathpar}
  \judgment{ }{
...
For addition expressions, we simply recursively mark each operand, which must analyse against
$\TNum$, and build the marked addition expression. In this way, blame for type errors is shift into
the operands:
%
\begin{mathpar}
  \judgment{
...
\end{mathpar}
%
Again, we need only synthetic rules; subsumption handles the analytic case.
...
the marked if-else expression from the results.
%
\begin{mathpar}
  \judgment{
...
%
\[\begin{array}{rrcl}
  \ECMName & \ECMV & \Coloneqq & \cdots \mid \ECInconBr{\ECMV}{\ECMV}{\ECMV}
\end{array}\]

Then, adding the following rules ensures marking totality on conditionals:
%
\begin{mathpar}
  \judgment{
    \ctxAnaFixedInto{\ctx}{\EMV_1}{\ECMV_1}{\TBool} \\
...
The extension of mark erasure is again straightforward:
%
\[\begin{array}{rcl}
  & \cdots & \\
...
\input{figs/calculus-products-judgments}
...
\input{figs/calculus-products-uexp}

\input{figs/calculus-products-mexp}

\input{figs/calculus-products-marking}
