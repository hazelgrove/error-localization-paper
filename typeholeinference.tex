...
Heuristic approaches, e.g. based on manually tuned weights\todo{cite}{} or machine learning methods \cite{SeidelBlame}, are necessary to guess which uses of a variable are consistent with the user's intent and which should be marked erroneous.
...
In order to generate global inference results, we begin with constraint generation. Our approach closely follows the usual approach \cite{TAPL0}\todo{cite other papers on gradual type inference}, where we augment the bidirectional type system for the marked lambda calculus with generated constraint sets, $C$. The new judgement forms are 
