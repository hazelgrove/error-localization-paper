...
At the core of expectations in typechecking expressions is analysis: when we expect something should have some type, we commonly analyze it against that type. For example, we analyze the scrutinees of if expressions against $\tbool$ and analyze the operands of addition against $\tnum$ as we expect they will have these types. Consequently, it is crucial that our analysis rules encapsulate these expectations in our constraints. For example, consider the following expression

For instance, consider the if expression $\EIf{\ECTrue}{\ehole^1}{\underlinenum{2}}$. When synthesizing its type, we expect that the scrutinee be of type $\tbool$ and thus analyze it against $\tbool$. We also expect that both branches of the if expression synthesize the consistent types and return their lower bound. 

when writing an if expression, we expect that both branches have the same or consistent types.

This leaves a few other cases (discuss matched arrow)
